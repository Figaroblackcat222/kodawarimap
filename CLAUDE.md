...
├── infrastructure/
│   ├── persistence/     # db.ts（Dexie v4, schema v12: hlcPhysical/hlcLogical/hlcNodeId/syncSchemaVersionをpins・photosに追加、sync_queueテーブル新規）,
...
│   │                    # auth-service.ts（JWT管理・自動リフレッシュ・並行リフレッシュは同一Promise共有。API_BASE=https://kodawarimap-api.figaroblackcat.workers.dev）,
│   │                    # cloudflare-sync-repository.ts（Workers API fetch実装・401時自動リフレッシュ再試行・写真はmultipart/form-data）,
...
└── presentation/
    ├── hooks/           # use-sync.ts（pull+push+pullPhotoSyncをtabCoordinator経由で実行・syncState管理・sync完了後にonSyncComplete通知）,
    │                    # use-key-derivation.ts（パスフレーズ→CryptoKey導出。saltはlocalStorage: kdm:sync-salt）
    └── components/      # map-view（useSync統合・encryptionKey state・起動時ログイン済みなら自動でSyncSetupSheet表示・sync完了時にrefreshLists()でUI再描画・
    ...
    │                    # sync-setup-sheet（初回登録/ログイン/パスフレーズ再入力の3モード・パスフレーズ紛失警告文・鍵導出後onSuccess(key)コールバック）,
    ...
    │                    # settings-sheet（地図情報更新（POIキャッシュclr+SW更新チェック）・ソート順・地図検索ON/OFF（Nominatim・同意ダイアログ付き）・ガイドメッセージON/OFF＋折りたたみ解除ボタン・昼夜自動テーマ切り替えトグル＋時刻設定・同期セクション（同期状態表示・今すぐ同期・パスフレーズ再入力・ログアウト）・バックアップセクション（最終エクスポート日時・30日未バックアップ時に警告バナー）），
...
│   ├── types.ts         # Env型（DB: D1Database, PHOTOS: R2Bucket, JWT_SECRET, ENVIRONMENT, CORS_ORIGIN）
│   ├── lib/
│   │   ├── jwt.ts       # HS256 sign/verify（crypto.subtle）
│   │   └── hlc.ts       # サーバー側HLC計算（computeServerHlc）
│   ├── middleware/
│   │   ├── cors.ts      # CORS（localhost常時許可・CORS_ORIGINと完全一致で本番許可）
│   │   └── auth.ts      # JWT検証ミドルウェア（requireAuth → {userId}）
│   └── routes/
│       ├── auth.ts      # POST /api/auth/register|login|refresh|logout・DELETE /api/account・ブルートフォース対策（login_attempts）
│       ├── pins.ts      # GET /api/pins/since・PUT /api/pins/:id（UPSERT）・DELETE /api/pins/:id（tombstone）
│       └── photos.ts    # GET /api/photos/list/:pinId・PUT /api/photos/:id（multipart R2アップロード）・GET /api/photos/:id（R2取得ETag対応）・DELETE /api/photos/:id
├── migrations/
...
└── wrangler.toml        # name: kodawarimap-api・D1: kodawarimap-sync・R2: kodawarimap-photos・cron: 0 3 * * *
                         # デプロイ先: https://kodawarimap-api.figaroblackcat.workers.dev
